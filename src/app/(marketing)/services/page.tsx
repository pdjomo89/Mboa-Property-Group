...
export default async function ServicesPage() {
  ...
  const services = [
    ...
    { icon: Receipt, title: tt.items.rent.title, body: tt.items.rent.body, gradient: "from-amber-500 to-orange-500", soft: "from-amber-50 to-orange-50", ring: "ring-amber-100", image: "/pexels-tima-miroshnichenko-6694536.jpg", fit: "contain" as const },
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {services.map(({ icon: Icon, title, body, gradient, soft, ring, image, fit }) => (
            ...
            >
              ...
              {image && (
                <div className={`relative h-48 w-full overflow-hidden ${fit === "contain" ? `bg-gradient-to-br ${soft}` : ""}`}>
                  <Image
                    src={image}
                    alt={title}
                    fill
